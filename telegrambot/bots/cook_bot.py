...
BOT_TOKEN = TELEGRAM_BOT_TOKENS['cook']
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if username in ALLOWED_TELEGRAM_USERNAMES:
        ...
        await update.message.reply_text("👨‍🍳 Доступ повару разрешён. Бот работает ✅")
        logger.info(f"Повар @{username} авторизован.")
    else:
...
async def check_new_orders(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{API_BASE_URL}/new_orders/?role=cook")
            ...
            for item in items:
                if item['id'] not in already_notified_ids:
                    ...
                    msg = (
                        f"👨‍🍳 <b>Заказ на кухню</b>\n"
                        f"🍽️ <b>Столик:</b> {table}\n"
                        f"🍲 <b>Блюдо:</b> {product} (x{quantity})\n"
                        f"⚙️ <b>Опции:</b>\n{options_text}\n"
...
def main():
    ...
    logger.info("Бот повара запущен.")
    app.run_polling()
